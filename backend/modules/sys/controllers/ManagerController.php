...
class ManagerController extends BaseController
{
    ...
    public function actionStorm(){
        ...
        /*获取全部门店信息*/
        $stormalls = Storm::find()->where('status >= 1')->asArray()->all();
        $stormall = array_column($stormalls,'storm_name','id');

        if ($model->load($request->post())) {
            $storm = $this->modelClass::findOne($id);
            $storm->storm_id = $request->post('Manager')['storm_id'];
            $data = $storm->save();//修改
            return $data
                ? $this->redirect(['index'])
                : $this->message($this->getError($model), $this->redirect(['index']), 'error');
